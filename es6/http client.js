...
var http_client = function () {
	...
	function http_client() {
		...
		var getCookie = this.server ? function (name) {
			...
			try {
				for (var _iterator = _getIterator(_this.set_cookies), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
		...
		var agent = this.server ? superagent.agent() : superagent;
		...
		try {
			var _loop = function _loop() {
				...
				_this[method] = function (path, data) {
					...
					var perform_http_request = function perform_http_request() {
						...
						var request = new Http_request(method, url, data, {
							...
							new_cookies: function new_cookies(_new_cookies) {
								if (_this.server) {
									...
									try {
										for (var _iterator3 = _getIterator(_new_cookies), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
						...
						return new _Promise(function (resolve, reject, onCancel) {
							// Send HTTP request
			...
			for (var _iterator2 = _getIterator(http_methods), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
				_loop();
			}
		} catch (err) {
...
function has_binary_data(data) {
	...
	try {
		for (var _iterator4 = _getIterator(_Object$keys(data)), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
...
var Http_request = function () {
	function Http_request(method, url, data, options) {
		_classCallCheck(this, Http_request);
		...
		if (data) {
			switch (method) {
				...
				case 'delete':
					throw new Error('"data" supplied for HTTP DELETE request: ' + _JSON$stringify(data));
	...
	_createClass(Http_request, [{
		...
		value: function add_cookies() {
			...
			if (set_cookies.size > 0) {
				...
				try {
					for (var _iterator6 = _getIterator(cookies_raw.split(';')), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
				...
				try {
					for (var _iterator7 = _getIterator(set_cookies), _step7; !(_iteratorNormalCompletion7 = (_step7 = _iterator7.next()).done); _iteratorNormalCompletion7 = true) {
				...
				cookies_raw = _Object$keys(cookies).map(function (key) {
