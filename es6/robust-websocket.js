...
export default function RobustWebSocket(url, protocols, userOptions) {
  var realWs = { close: function close() {} },
      ...
      pendingReconnect,
      opts = _Object$assign({}, RobustWebSocket.defaultOptions, typeof userOptions === 'function' ? { shouldReconnect: userOptions } : userOptions);
  ...
  ['bufferedAmount', 'url', 'readyState', 'protocol', 'extensions'].forEach(function (readOnlyProp) {
    _Object$defineProperty(self, readOnlyProp, {
      get: function get() {
  ...
  function newWebSocket() {
    ...
    attempts++;
    self.dispatchEvent(_Object$assign(new CustomEvent('connecting'), {
      attempts: attempts,
    ...
    connectTimeout = setTimeout(function () {
      connectTimeout = null;
      detachConnectivityEvents();
      self.dispatchEvent(_Object$assign(new CustomEvent('timeout'), {
        attempts: attempts,
        reconnects: reconnects
      }));
