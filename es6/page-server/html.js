...
export default function Html(options) {
	...
	try {
		for (var _iterator = _getIterator(assets.entries), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
	...
	var webpage_head = server_side_generated_webpage_head();

	return template.render(_extends({}, options, {
		webpage_head: webpage_head,
		style_urls: style_urls,
		javascript_urls: javascript_urls,
		get_language_from_locale: get_language_from_locale,
...
export function safe_json_stringify(json) {
	...
	return _JSON$stringify(json).replace(/\//g, '\\/');
}
