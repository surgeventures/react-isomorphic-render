...
exports.default = authorized_route;
...
//
function authorized_route(get_user, authorization, Wrapped) {
	...
	(0, _hoistNonReactStatics2.default)(Authorized, Wrapped);

	var preload = Authorized[_preloadingMiddleware.Preload_method_name];
	...
	Authorized[_preloadingMiddleware.Preload_method_name] = function authorize_then_preload(parameters) {
		var getState = parameters.getState;


		var error = check_privileges(get_user(getState()), authorization);

		if (error) {
			throw_error(error);
		}

		return preload ? preload(parameters) : _promise2.default.resolve();
	};
...
function throw_error(error_code) {
	var error = new Error(error_code);
